<x-layouts.app title="Admin - Nouvelle Animation">
        ...
        <div class="max-w-4xl mx-auto px-4">
            ...
            <div class="mb-8">
                <h1 class="text-3xl font-black text-soboa-blue flex items-center gap-3">
                    <span class="text-4xl">➕</span> Nouvelle Animation
                </h1>
                <p class="text-gray-600 mt-2">Créez une nouvelle animation pour un point de vente</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-8">
                <form action="{{ route('admin.store-animation') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-6">
                        ...
                        <select name="bar_id" required
                            ...
                            @foreach($bars as $bar)
                                <option value="{{ $bar->id }}" {{ old('bar_id', request('bar_id')) == $bar->id ? 'selected' : '' }}>
                                    {{ $bar->name }}
                                    @if($bar->zone)
                                        ({{ $bar->zone }})
                                    @endif
                    ...
                    <div class="mb-6">
                        ...
                        <select name="phase"
                            ...
                            <option value="quarter_final" {{ old('phase') == 'quarter_final' ? 'selected' : '' }}>Quart de finale</option>
                            <option value="semi_final" {{ old('phase') == 'semi_final' ? 'selected' : '' }}>Demi-finale</option>
                            <option value="third_place" {{ old('phase') == 'third_place' ? 'selected' : '' }}>3e place</option>
                    ...
                    <div class="flex items-center gap-4 pt-6 border-t">
                        <button type="submit"
                            ...
                            <span>💾</span> Créer l'animation
                        </button>
                        <a href="{{ route('admin.animations') }}"
